...
# ── Calibration data from diagnosis ─────────────────────────────────────
# Format: (raw_predicted_probability, actual_observed_rate)
_CALIBRATION_POINTS = [
...
# Sort calibration points by predicted probability for piecewise linear interpolation
_CALIBRATION_POINTS.sort(key=lambda x: x[0])

def _calibrate_probability(raw_proba: float) -> float:
    """
    Apply piecewise linear calibration to raw ensemble probability.
    Uses diagnosis data to map miscalibrated probabilities to observed frequencies.
    """
    if not _CALIBRATION_POINTS:
        return raw_proba
        
    # Handle edge cases
    if raw_proba <= _CALIBRATION_POINTS[0][0]:
        # Below first point - extrapolate using first two points if available
        if len(_CALIBRATION_POINTS) >= 2:
            x1, y1 = _CALIBRATION_POINTS[0]
            x2, y2 = _CALIBRATION_POINTS[1]
            return y1 + (raw_proba - x1) * (y2 - y1) / (x2 - x1)
        else:
            return _CALIBRATION_POINTS[0][1]
            
    if raw_proba >= _CALIBRATION_POINTS[-1][0]:
        # Above last point - extrapolate using last two points if available
        if len(_CALIBRATION_POINTS) >= 2:
            x1, y1 = _CALIBRATION_POINTS[-2]
            x2, y2 = _CALIBRATION_POINTS[-1]
            return y1 + (raw_proba - x1) * (y2 - y1) / (x2 - x1)
        else:
            return _CALIBRATION_POINTS[-1][1]
    
    # Find interval containing raw_proba
    for i in range(len(_CALIBRATION_POINTS) - 1):
        x1, y1 = _CALIBRATION_POINTS[i]
        x2, y2 = _CALIBRATION_POINTS[i + 1]
        if x1 <= raw_proba <= x2:
...
def _build_single_row_features(event: dict) -> pd.DataFrame:
    ...
    `event` is expected to have raw fields like the original dataset:
    ...
    # build_features() expects 'severity' to already exist (it's normally
    # added by add_severity_target_binary at training time). At inference
    # time we don't know the answer yet -- inject a placeholder column so
    # the shared pipeline code runs unmodified, then drop it.
    df["severity"] = "routine"  # placeholder, value is irrelevant and discarded below
    ...
    # Reorder columns to EXACTLY match what the models were trained on
    # (see TRAINED_FEATURE_ORDER comment above) -- this is what fixes the
    # "feature_names mismatch" error from XGBoost.
    return features[TRAINED_FEATURE_ORDER]


def _ensemble_proba_high(X: pd.DataFrame) -> float:
    """Average P(high) across the 3 models. Returns a single float."""
    ...
    # Apply calibration to improve probability interpretability
    calibrated_ensemble = _calibrate_probability(ensemble)
    return calibrated_ensemble, individual
